...
/**
 * The Class representing a complete game.QuadTree, most operations are implemented
 ...
 * @param center  The center of the game.QuadTree
 ...
 */
class QuadTree[A](K: Int = 2, center: Point = Point(0, 0), halfDim: Int = 16) {
    ...
    def knnSearch(p: Point, k: Integer): Seq[(Point, A)] = {
        if (k <= 0) return new ListBuffer[(Point, A)]().toSeq
        ...
        knnElements.toSeq.map(Element.unapply(_).get)
    }
    ...
    class Node(K: Int, var bounds: Box = null) {
        ...
        def children: Array[Node] = Array(topLeft, topRight, bottomLeft, bottomRight) // For iterative access
         ...
         *
         * Recursively performs insertion to the game.QuadTree by
        ...
        def insert(elem: Element): Boolean = {
            ...
            if (!bounds.contains(elem.position)) {
                throw new IllegalArgumentException(s"The position '${elem.position}' is out of game.QuadTree bounds.")
            }
         ...
         * the game.QuadTree nodes until it reaches a leaf node. If the leaf
        ...
        def remove(point: Point): Boolean = {
            def mustCollapse = !isLeaf && children.forall(_.isLeaf) && children.foldLeft(0) {
                _ + _.elements.size
            } <= K

            def collapse(): Unit = {
                elements = children.foldLeft(ListBuffer[Element]()) {
                    _ ++ _.elements
                }
                topLeft = null
                topRight = null
        ...
        def rangeSearch(fromPos: Point, toPos: Point): ListBuffer[Element] = {
            ...
            if (isLeaf) {
                ...
                children
                  ...
                  .foldLeft(ListBuffer[Element]()) {
                      _ ++ _.rangeSearch(fromPos, toPos)
                  }
            }
        }
